...
function odil_lbfgs(problem::Odil; max_iterations = 100000, extra = problem.extra, p_extra = problem.p_extra, len_extra = problem.len_extra, u_iter0 = problem.u_iter0, autodiff = AutoEnzyme())
    return odil_lbfgs(problem.lhs, problem.rhs, problem.p_lhs, problem.p_rhs, problem.N_coords, problem.u_reference_vals, problem.reference_val_indices, problem.t; max_iterations = max_iterations, extra = extra, p_extra = p_extra, len_extra = len_extra, u_iter0 = u_iter0, autodiff = autodiff)
end

odil_lbfgs(problem::Odil1D; kwargs...) = odil_lbfgs(base_problem(problem); kwargs...)
odil_lbfgs(problem::Odil2D; kwargs...) = odil_lbfgs(base_problem(problem); kwargs...)
odil_lbfgs(problem::Odil3D; kwargs...) = odil_lbfgs(base_problem(problem); kwargs...)
