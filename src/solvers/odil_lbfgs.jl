using SciMLBase, Optim, ADTypes, Enzyme, LinearAlgebra

"""
Solve an ODIL problem with an L-BFGS optimizer.
"""
function odil_lbfgs(problem::OdilProblem; max_iterations = 100000, info_prints = true, callback_set = OdilCallbackSet())
    return odil_lbfgs(problem.timestep, problem.p_timestep, problem.N_coords, problem.u_reference_vals, problem.reference_val_indices, problem.t; max_iterations = max_iterations, timestep_alloc_size = problem.timestep_alloc_size, extra = problem.extra, p_extra = problem.p_extra, len_extra = problem.len_extra, u_iter0 = problem.u_iter0, problem = problem, info_prints = info_prints, callback_set = callback_set)
end

"""
Solve the ODIL problem directly from its components using a limited-memory BFGS method.
"""
function odil_lbfgs(timestep, p_timestep, N_coords, u_reference_vals, reference_val_indices, t; max_iterations = 100000, timestep_alloc_size = 0, extra = nothing,  p_extra = nothing, len_extra = 0, u_iter0 = nothing, problem = nothing, info_prints = true, callback_set = OdilCallbackSet())
    Nref = length(u_reference_vals)
    Nt = length(t)
    num_unknowns = N_coords * Nt
    iter = Ref(0)
    if u_iter0 === nothing
        u_iter0 = zeros(num_unknowns)
    end

    p_all = (timestep, p_timestep, timestep_alloc_size, u_reference_vals, reference_val_indices, Nref, N_coords, Nt, t, extra, p_extra, len_extra, iter)

    function loss(u_vec, p)
        timestep_inner, p_timestep_inner, timestep_alloc_size_inner, u_reference_vals_inner, reference_val_indices_inner, Nref_inner, N_coords_inner, Nt_inner, t_inner, extra_inner, p_extra_inner, len_extra_inner, iter_inner = p
        l = zero(eltype(u_vec))
        l_exact = zero(eltype(u_vec))
        l_pde = zero(eltype(u_vec))

        for i in eachindex(u_reference_vals_inner)
            u_val = u_reference_vals_inner[i]
            idx = reference_val_indices_inner[i]
            l_exact += ((u_vec[idx] - u_val)^2)/Nref_inner
        end
        
        u_timestep = zeros(eltype(u_reference_vals), N_coords_inner * (Nt_inner - 1))
        timestep_mem = zeros(eltype(u_reference_vals), timestep_alloc_size_inner * (Nt_inner - 1))

        for it in 2:Nt_inner
            u_timestep_it = @view(u_timestep[(it - 2) * N_coords_inner + 1:(it - 1) * N_coords_inner])
            u_timestep_it .= zero(eltype(u_vec))
            timestep_mem_it = @view(timestep_mem[(it - 2) * timestep_alloc_size_inner + 1:(it - 1) * timestep_alloc_size_inner])
            timestep_mem_it .= zero(eltype(u_vec))
            # fill!(u_timestep, zero(eltype(u_vec)))
            # fill!(timestep_mem, zero(eltype(u_vec)))
            u_it_last = @view(u_vec[(it - 2) * N_coords_inner + 1:(it - 1) * N_coords_inner])
            u_it = @view(u_vec[(it - 1) * N_coords_inner + 1:it * N_coords_inner])
            timestep_inner(timestep_mem_it, u_timestep_it, u_it_last, t_inner[it - 1], t_inner[it] - t_inner[it - 1], p_timestep_inner)
            l_pde += sum((u_timestep_it - u_it).^2)/(N_coords_inner * Nt_inner)
        end

        l = l_exact + l_pde

        if extra_inner !== nothing && p_extra_inner !== nothing
            du_extra = zeros(eltype(u_reference_vals), len_extra_inner)
            extra_inner(du_extra, u_vec, p_extra_inner, iter_inner[])
            l_extra = sum(du_extra.^2)/(len_extra_inner)
            l += l_extra
        end

        return l
    end
    
    # optf = OptimizationFunction(loss, autodiff, grad = true, hess = false)
    # prob = OptimizationProblem(optf, u_iter0, p_all) 
    # opt = LBFGS(m = 50)
    opt = LBFGS()
    f = (u_vec) -> loss(u_vec, p_all)
    df = Enzyme.make_zero(f)
    optf = Enzyme.Duplicated(f, df)

    if info_prints
        add!(callback_set, InfoPrintCallback())
    end

    callback = function (state)
        iter[] += 1
        callback_set(problem, state.x, state.f_x, state.g_x, iter[])
        return false
    end
    
    if info_prints
        println("Starte Lösung...")
    end
    # res = solve(prob, opt, maxiters = max_iterations, callback = callback)
    res = optimize(optf, u_iter0, opt, Optim.Options(iterations = max_iterations, callback = callback, g_tol = 1e-12); autodiff = AutoEnzyme())
    
    if info_prints
        println("Optimierung beendet!")
        println("Return Code: ", res.termination_code)
    end
    return res.minimizer
end