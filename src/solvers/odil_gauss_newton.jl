...
function odil_gauss_newton(timestep, p_timestep, N_coords, u_reference_vals, reference_val_indices, t; max_iterations = 2, timestep_alloc_size = 0, extra = nothing,  p_extra = nothing, len_extra = 0, u_iter0 = nothing, problem = nothing, info_prints = true, jac_sparse = nothing, colors = nothing)
    ...
    p_iter = Ref(0)
    if u_iter0 === nothing
        u_iter0 = zeros(N_coords * Nt)
    end

    p_all = (timestep, p_timestep, timestep_alloc_size, u_reference_vals, reference_val_indices, Nref, N_coords, Nt, t, extra, p_extra, len_extra, p_iter)
    resid_prototype = zeros(eltype(u_iter0), Nref + N_coords * (Nt - 1) + len_extra)

    function operator_loss!(du, u_vec, p)
        timestep_inner, p_timestep_inner, timestep_alloc_size_inner, u_reference_vals_inner, reference_val_indices_inner, Nref_inner, N_coords_inner, Nt_inner, t_inner, extra_inner, p_extra_inner, len_extra_inner, iter_inner = p
        ...
        du[Nref_inner + 1:Nref_inner + N_coords_inner * (Nt_inner - 1)] .= zero(eltype(u_vec))
        l_pde = @view(du[Nref_inner + 1:Nref_inner + N_coords_inner * (Nt_inner - 1)])

        u_timestep = zeros(eltype(u_vec), N_coords_inner)
        ...
        for it in 2:Nt_inner
            ...
            u_it_last = @view(u_vec[(it - 2) * N_coords_inner + 1:(it - 1) * N_coords_inner])
            u_it = @view(u_vec[(it - 1) * N_coords_inner + 1:it * N_coords_inner])
            timestep_inner(timestep_mem, u_timestep, u_it_last, t_inner[it - 1], t_inner[it] - t_inner[it - 1], p_timestep_inner)
            l_pde[(it - 2) * N_coords_inner + 1: (it - 1) * N_coords_inner] .= (u_timestep - u_it)/sqrt(N_coords_inner * Nt_inner)
        end

        du[Nref_inner + N_coords_inner * (Nt_inner - 1) + 1:end] .= zero(eltype(u_vec))
        l_extra = @view(du[Nref_inner + N_coords_inner * (Nt_inner - 1) + 1:end])
